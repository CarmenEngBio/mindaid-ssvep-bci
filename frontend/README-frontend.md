# Frontend Manual - Browser Endpoint explanation
 ...
 1. It is showed first an instructions menu:
     - It appears when loading the website. Closes by pressing the `Got it! - Begin` button. 
     - The menu covers the screen where the keypad is being placed, so the `Start Session` button to begin the recording is only possible if the instructions
       modal menu is closed.
 ...
 The `.key.selected` displays a green color if the cell was correctly chosen, providing to the user evaluations feedback. 
...
---
 ...
 - Each emoji flickers at its frequency (4 different assigned to each cell) by calling the requestAnimationFrame for temporal accuracy;
   considering as well the screen refresh rate of 60 Hz in the laptop where experiments were conducted.
 - The frequencies are read from the `data-freq` attribute taken from the **DOM** (built from the index.html when it is loaded).
 - The period is linked to the ms per half cycle due to the on-off transitions. When the cell is asigned to be turned off it is seen as
   black and when it is on it is seen as white; due to the frequency of oscillation been short it is seen by the human eye as "cell-blinking"
   oscillations.
 ...
 - **clearCellSelection**: removes the green feedback color before each new iteration result and so that the cell is seen as usual white color appearance. 
 - **showMessage**: displays in a blue color which is the target cell to gaze at and in green if the objective was selected. 
 - **startCountdown**: shows the seconds remaining for the end of the recording at each iteration (backwards timer).  
 ...
 - **connect**: this function opens the connection between the browser and the server and reattempts 2s later if it was not achieved.
 ...
 - **handleSessionEnded**: finishes the countdown, clears the selection for the next recording session and shows the accuracy rate in
   percentage and respect to the total number of processed evaluated blocks. It restores the UI such that a new session can be selected
   by pressing the Start Button available again.
